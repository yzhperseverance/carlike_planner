...
namespace uneven_planner
{
    RXS2 UnevenMap::filter(Eigen::Vector3d pos, vector<Eigen::Vector3d> points)
    {
        ...
        for (size_t i=0; i<points.size(); i++)
        {
            ...
            cov += v * v.transpose();// 协方差Cov=(1/m)∑eeT
        }
        ...
        Eigen::MatrixXd::Index evalsMax; // 最小特征值的索引
        D.minCoeff(&evalsMax); // 寻找最小特征值，即z轴对应方向
        ...
        rs2.sigma = D(evalsMax) / D.sum() * 3.0; // 论文里写到是用来计算坡度的
        if (isnan(rs2.sigma))
        {
            rs2.sigma = 1.0;
            n = Eigen::Vector3d(1.0, 0.0, 0.0);
        }
        rs2.z = mean_points.z(); // 直接平均值准吗？
        // zb是单位向量，并且属于S2+，所以只给出两个坐标（zb不是三维向量吗？）
        rs2.zb.x() = n(0, 0);
        rs2.zb.y() = n(1, 0);
    ...
    void UnevenMap::init(ros::NodeHandle& nh)
    {
        ...
        // construct map: SE(2) --> RXS2
        if (!constructMapInput()) // 判断是否已经构建过地图信息了
            constructMap();
        
        // occ map 这里好像没有考虑障碍物？如果想用在别的场景应该要改
    ...
    bool UnevenMap::constructMap()
    {
        ...
        // 当x和y确定时，由于地形的起伏，不同的yaw所对应的zb不同，因为车是有长度的，在坡上原地旋转一周所对应的zb是不同的
        // 这样分析的话这个椭球的区域应该与车的大小相关？
        for (int x=0; x<voxel_num[0]; x++)
            for (int y=0; y<voxel_num[1]; y++)
                for (int yaw=0; yaw<voxel_num[2]; yaw++)
                    for (int iter=0; iter<iter_num; iter++)
                    {
                        Eigen::Vector3d map_pos;// 物理坐标
                        RXS2 map_rs2 = map_buffer[toAddress(x, y, yaw)]; // 第0次迭代时是0
                        ...
                        world_pos.head(2) += xb.head(2) * 0.12; // 这是在干嘛？？？
                        ...
                        if (kd_tree.radiusSearch(pt, box_r, Idxs, SquaredDists) > 0)
                        {
                            ...
                            for (size_t i=0; i<Idxs.size(); i++)
                            {
                                ...
                                Eigen::Vector3d temp_inrob = RT*temp_subtract; // 新local系下的坐标
                                // 椭球公式x/a + y/b + z/c = 1，由于kd树取的是球内的点，所以这里还要再取一下椭球内的点
